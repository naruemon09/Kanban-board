<template>
  <div class="relative">
    <div class="absolute top-4 right-4 flex gap-2 z-10">
      <button
        @click="editing(board)"
        class="p-2 bg-white/90 backdrop-blur-sm rounded-lg hover:bg-white transition-all duration-200 shadow-sm hover:shadow-md group/edit"
        title="Edit Board"
      >
        <svg class="w-5 h-5 text-gray-700 group-hover/edit:text-indigo-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        title="Delete Board"
      >
        <svg class="w-5 h-5 text-gray-700 group-hover/delete:text-red-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="p-6 pt-16">
      <template v-if="board.isEditing">
        <div class="space-y-3">
          ...
          <div class="flex gap-2">
            <button @click="saveEdit(board)" class="flex-1 px-4 py-2.5 text-white font-medium bg-green-600 rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center gap-2">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
              Save
            </button>
            <button @click="cancelEdit(board)" class="flex-1 px-4 py-2.5 text-gray-700 font-medium bg-white rounded-lg hover:bg-gray-100 transition-colors">
              Cancel
            </button>
          </div>
        </div>
      ...
      <template v-else>
        <h5 class="text-2xl font-bold text-gray-900 mb-6 line-clamp-2 min-h-[3.5rem]">
          {{ board.boardName }}
        </h5>
        ...
        >
          ...
          <svg class="w-4 h-4 ml-2 transform group-hover/view:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  methods: {
    ...
    async saveEdit(board) {
      try {
        ...
        const response = await updateBoard(token, board.id, {boardName : board.newName});
